...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 20 13:39:34 2025

PJD 20 Feb 2025 - creating a generic function library for use in DRCDP,
                  building on obs4MIPsLib
PJD 20 Feb 2025 - added getCVs, writeUserJson; updated writeJson    
...
def getCVs(tableFilePath):
    """
    From tableFilePath ascertain path to CVs, load and return
    DRS dictionary and tracking_id prefix

    Parameters
    -------
    tableFilePath : table file path

    Returns
    -------
    tablePath : local path to Tables subdir
    DRS : python dictionary with directory_path and filename templates
    trackingId : python dictionary with tracking_id prefix
    """
    tablePath = os.path.dirname(tableFilePath)
    # locate CVs upper level, map path, load and return dictionaries
    DRSPath = tablePath.replace("Tables", "DRCDP_DRS.json")
    print("DRSPath:", DRSPath)
    with open(DRSPath) as f:
        DRS = json.load(f)
    trackingIdPath = tablePath.replace("Tables", "DRCDP_tracking_id.json")
    with open(trackingIdPath) as f:
        trackingId = json.load(f)

    return tablePath, DRS, trackingId
...
def writeJson(dataDict, outFilePath):
    """
    Given a json data dictionary, write out to provided filename

    Returns
    -------
    path : return outFilePath
    """
...
def writeUserJson(userJsonPath, tableFilePath):
    """
    Given user json dictionary, append CMOR-required attributes managed by DRCDP CVs

    Returns
    -------
    userJsonPath : local path to function-generated file
    """
    # get CV-managed content
    tablePath, DRS, tracking_id = getCVs(tableFilePath)

    # load userJson
    with open(userJsonPath, "r") as f:
        userDict = json.load(f)

    # write additional entries to dictionary
    # the following are DRCDP project specific settings
    userDict["tracking_prefix"] = tracking_id["tracking_id_prefix"][0]
    userDict["output_file_template"] = DRS["DRS"]["filename_template"]
    userDict["output_path_template"] = DRS["DRS"]["directory_path_template"]
    # the following are CMOR internal settings
    userDict["_history_template"] = (
        "%s; CMOR rewrote data to be consistent with <activity_id>, CMIP6, CMIP6Plus and <Conventions> standards"
    )
    # test file existence
    if os.path.exists(os.path.join(tablePath, "DRCDP_coordinate.json")):
        userDict["_AXIS_ENTRY_FILE"] = (
            "DRCDP_coordinate.json"  # assumes same path as TableFilePath
        )
        userDict["_FORMULA_VAR_FILE"] = "DRCDP_formula_terms.json"
        userDict["_controlled_vocabulary_file"] = "DRCDP_CV.json"
    else:
        print("DRCDP_coordinate.json file not found, exiting")
        sys.exit()

    # write revised dictionary to file
    path = writeJson(userDict, "userInput.json")
